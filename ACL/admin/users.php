		...
		case 'saveRoles':
			...
			foreach ($_POST as $k => $v)
			{
				if (substr($k,0,5) == "role_")
				{
					...
					if ($v == '0' || $v == 'x') {
						$strSQL = sprintf("DELETE FROM `user_roles` WHERE `user_id` = %u AND `role_id` = %u",$_POST['userID'],$roleID);
					} else {
						$strSQL = sprintf("REPLACE INTO `user_roles` SET `user_id` = %u, `role_id` = %u, `addDate` = '%s'",$_POST['userID'],$roleID,date ("Y-m-d H:i:s"));
					}
		...
		case 'savePerms':
			...
			foreach ($_POST as $k => $v)
			{
				if (substr($k,0,5) == "perm_")
				{
					...
					if ($v == 'x')
					{
						$strSQL = sprintf("DELETE FROM `user_perms` WHERE `user_id` = %u AND `permission_id` = %u",$_POST['userID'],$permID);
					} else {
						$strSQL = sprintf("REPLACE INTO `user_perms` SET `user_id` = %u, `permission_id` = %u, `value` = %u, `addDate` = '%s'",$_POST['userID'],$permID,$v,date ("Y-m-d H:i:s"));
					}
					mysqli_query($con,$strSQL);
				}
			}
...
<div id="page">
	<? if ($_GET['action'] == '' ) { ?>
		...
		{
			echo "<a href=\"?action=user&userID=" . $row['user_id'] . "\">" . $row['user_username'] . "</a><br />";
		}
    } ?>
    <?
    if ($_GET['action'] == 'user' ) { 

...
if (isset($_POST['action']))
{
	...
	{
		case 'savePerm':
			$strSQL = sprintf("REPLACE INTO `permissions` SET `permission_id` = '%u', `permission_name` = '%s', `permission_key` = '%s'",$_POST['permID'],$_POST['permName'],$_POST['permKey']);
			mysqli_query($con, $strSQL);
		break;
		case 'delPerm':
			$strSQL = sprintf("DELETE FROM `permissions` WHERE `permission_id` = %u LIMIT 1",$_POST['permID']);
			mysqli_query($con, $strSQL);
		break;
	}

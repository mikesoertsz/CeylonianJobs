...
if (isset($_POST['action']))
{
	...
	{
		case 'saveRole':
			$strSQL = sprintf("REPLACE INTO `roles` SET `role_id` = %u, `role_name` = '%s'",$_POST['roleID'],$_POST['roleName']);
			...
			foreach ($_POST as $k => $v)
			{
				if (substr($k,0,5) == "perm_")
				{
					...
					if ($v == 'X')
					{
						$strSQL = sprintf("DELETE FROM `role_perms` WHERE `role_id` = %u AND `permission_id` = %u",$roleID,$permID);
						mysqli_query($con,$strSQL);
						continue;
					}
					$strSQL = sprintf("REPLACE INTO `role_perms` SET `role_id` = %u, `permission_id` = %u, `role_perm_value` = %u, `role_perm_addDate` = '%s'",$roleID,$permID,$v,date ("Y-m-d H:i:s"));
					mysqli_query($con,$strSQL);
				}
			}
		...
		break;
		case 'delRole':
			$strSQL = sprintf("DELETE FROM `roles` WHERE `role_id` = %u LIMIT 1",$_POST['roleID']);
			mysqli_query($con,$strSQL);
			$strSQL = sprintf("DELETE FROM `user_roles` WHERE `role_id` = %u",$_POST['roleID']);
			mysqli_query($con,$strSQL);
			$strSQL = sprintf("DELETE FROM `role_perms` WHERE `role_id` = %u",$_POST['roleID']);
			mysqli_query($con,$strSQL);
			header("location: roles.php");
		break;
	}
